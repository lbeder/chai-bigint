...
function supportBigInt(Assertion: Chai.AssertionStatic, utils: Chai.ChaiUtils) {
...
export default function chaiBigInt(chai: Chai.ChaiStatic, utils: Chai.ChaiUtils) {
...
function override(method: Methods, name: string, utils: Chai.ChaiUtils) {
  return (_super: (...args: any[]) => any) => overwriteBigIntFunction(method, name, _super, utils);
}
...
function overwriteBigIntFunction(
  ...
  return function (this: Chai.AssertionStatic, ...args: any[]) {
    ...
    if (isBigInt(actual) || isBigInt(expected)) {
      ...
      this.assert(
        cond,
        `Expected "${actual}" to be ${readableName} ${expected}`,
        `Expected "${actual}" NOT to be ${readableName} ${expected}`,
        actual,
        expected
      );
    } else {
...
function overwriteBigNumberWithin(_super: (...args: any[]) => any, chaiUtils: Chai.ChaiUtils) {
  return function (this: Chai.AssertionStatic, ...args: any[]) {
    ...
    if (isBigInt(actual) || isBigInt(start) || isBigInt(finish)) {
      this.assert(
        BigInt(start) <= actual && BigInt(finish) >= actual,
        `Expected "${actual}" to be within [${[start, finish]}]`,
        `Expected "${actual}" NOT to be within [${[start, finish]}]`,
        [start, finish],
        actual
      );
    } else {
...
function overrideCloseTo(utils: Chai.ChaiUtils) {
  return (_super: (...args: any[]) => any) => overwriteBigNumberCloseTo(_super, utils);
}
...
function overwriteBigNumberCloseTo(_super: (...args: any[]) => any, chaiUtils: Chai.ChaiUtils) {
  return function (this: Chai.AssertionStatic, ...args: any[]) {
    ...
    if (isBigInt(actual) || isBigInt(expected) || isBigInt(delta)) {
      this.assert(
        BigInt(actual) >= BigInt(expected) - BigInt(delta) && BigInt(actual) <= BigInt(expected) + BigInt(delta),
        `Expected "${actual}" to be within ${delta} of ${expected}`,
        `Expected "${actual}" NOT to be within ${delta} of ${expected}`,
        `A number between ${BigInt(expected) - BigInt(delta)} and ${BigInt(expected) - BigInt(delta)}`,
        actual
      );
    } else {
      _super.apply(this, args);
